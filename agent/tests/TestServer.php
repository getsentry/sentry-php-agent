...
trait TestServer
{
     ...
     */
    protected static $serverOutputFile = __DIR__ . '/fixtures/testserver/output.json';

    /**
     * @var string the path to the request count file
     */
    protected static $serverRequestCountFile = __DIR__ . '/fixtures/testserver/request_count.txt';
    ...
    public function startTestServer(): string
    {
        ...
        if (file_exists(self::$serverRequestCountFile)) {
            unlink(self::$serverRequestCountFile);
        }
        ...
        $this->serverProcess = proc_open(
            $command = \sprintf(
                ...
                $this->serverPort,
                realpath(__DIR__ . '/fixtures/testserver')
            ),
            [2 => ['pipe', 'w']],
            $pipes
        );
